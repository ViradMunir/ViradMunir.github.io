...
> {
  ...
  disabled?: boolean;
  /** Extra room (px) above the wheel so scaled/hovered items are not clipped. */
  topSpace?: number;
}
        ...
        >
          ...
          >
            {childrenNodes.map((child, index) => {
              // Rounded so the server-rendered transform string matches the
              // browser-normalised one (avoids a hydration warning).
              const round = (n: number) => Number(n.toFixed(3));
              const angle = (index / childrenCount) * 2 * Math.PI;
              let x = round(currentRadius * Math.cos(angle));
              const y = round(currentRadius * Math.sin(angle));

              if (direction === "rtl") {
                x = -x;
              }

              const rotationAngle = round((angle * 180) / Math.PI);
